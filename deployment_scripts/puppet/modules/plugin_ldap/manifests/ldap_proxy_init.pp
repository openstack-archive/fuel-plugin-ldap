class plugin_ldap::ldap_proxy_init (
  ...
  $slapd_rsyslog           = '/etc/rsyslog.d/slapd.log',
  $slapd_rsyslog_template  = '/etc/fuel/plugins/ldap-3.0/puppet/modules/plugin_ldap/manifests/templates/slapd_rsyslog.erb',
) {
  ...
  File_line['slapd_defaults'] -> Exec['run_slaptest'] ~> Service['slapd']

  file { "${slapd_rsyslog}":
    ensure => present,
    content => $slapd_rsyslog_template,
  }
